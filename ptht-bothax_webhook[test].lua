...
function sendWebhook(status, color)
    RunThread(function()
        MakeRequest(
            WEBHOOK_URL,
            "POST",
...
function startFarm()
    ...
    for i = 1, count_ptht do
        if not isRunning then
            sendWebhook("STOPPED (Manual)", 15158332)
            break
        end
        ...
        if magplantEmpty then
            sendWebhook("MAGPLANT EMPTY!", 16776960)
        end
        ...
        sendWebhook("RUNNING - Cycle " .. done .. "/" .. count_ptht, 10181046)
